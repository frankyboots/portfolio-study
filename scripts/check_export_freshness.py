...
def main(argv: list[str] | None = None) -> int:
    ...
    record_paths = sorted(
        p
        for p in (
            staged_figdir.glob(f"*{RECORD_SUFFIX}") if staged_figdir.is_dir() else []
        )
    )
    if not record_paths:
        print(
    ...
    for record_path in record_paths:
        ...
        new_svg = staged_svg.read_bytes()
        disk_svg_bytes = disk_svg.read_bytes()
        new_svg_text = new_svg.decode("utf-8")
        if not SVG_BUILD_HEAD_RE.search(new_svg_text):
            problems.append(f"{where}SVG stamp line missing from regenerated export")
        try:
            if _diff.strip_stamp_groups(new_svg) != _diff.strip_stamp_groups(
                disk_svg_bytes
            ):
        ...
        try:
            if not _diff.png_equal_outside_rect(
                staged_png.read_bytes(), disk_png.read_bytes(), style.STAMP_RECT_PX
            ):
                problems.append(
        ...
        # Record: fields equal with the stamp excluded; VINTAGE matches the sidecar.
        try:
